...
void control_camera(first_person_camera_t* camera, GLFWwindow* window, uint32_t* reset_accum) {
	...
	if (camera->rotate_camera) {
		...
		// Reset accumulation if there is translation
		*reset_accum = 1;	
	}
	...
	// Reset accumulation if there has been translation
	if (fabs(forward) > 0.0 || fabs(right) > 0.0 || fabs(vertical) > 0.0)
		*reset_accum = 1;	
