...
class Mesh:
    ...
    def __init__(self):
        ...
        self.vertex_light = np.zeros(0, dtype=np.uint32)    # Store whether vertex is part of a light

    @staticmethod
    def from_blender_mesh(mesh, line_radius, is_light):
        """
        ...
        :param line_radius: If there are no polygons, the mesh is treated as
            ...
            for such meshes.
        :param is_light: Is the given mesh a light source
        ...
        result.vertex_light = np.full(len(mesh.vertices), 1 if is_light else 0, dtype=np.uint32)
        if not line_mode:
    ...
    def merge_meshes(lhs, rhs, lhs_material_slot_remap=None, rhs_material_slot_remap=None):
        ...
        result.vertex_light = np.concatenate([lhs.vertex_light, rhs.vertex_light])
    ...
    def get_material_sub_mesh(self, material_slot_index):
        ...
        result.vertex_light = np.zeros(remaining_vertex_indices.size, dtype=np.uint32)
        result.vertex_light = self.vertex_light[remaining_vertex_indices]
        for j in range(3):
...
class Scene:
    ...
    def __init__(self, scene, selection_only, add_triangulate_modifier, split_angle, line_radius, light_collection):
        """
        ...
        def blender_object_to_mesh(obj, first_try):
            ...
            is_light = obj.name in bpy.data.collections[light_collection].all_objects
            result = Mesh.from_blender_mesh(blender_mesh, line_radius, is_light)
...
def export_scene(blender_scene, scene_file_path, selection_only, add_triangulate_modifier, split_angle, sort_triangles, light_collection):
    """
    ...
    scene = Scene(blender_scene, selection_only, add_triangulate_modifier, split_angle, None, light_collection)
    ...
    used_material_list = [re.sub(r"_c4d.*$", "", name) for name in used_material_list]
    ...
    # Set the last remaining bit to signify that this is a light
    packed_positions[:, 1] += mesh.vertex_light << 31
...
class VulkanExportOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        export_scene(context.scene, export_path, self.selection_only, self.add_triangulate_modifier,
                     ...
                     self.sort_triangles, self.light_collection)
        return {"FINISHED"}
    ...
    sort_triangles: bpy.props.BoolProperty(name="Sort triangles",
                                           ...
                                           default=True)
    light_collection: bpy.props.StringProperty(name="Light collection",
                                           description="Which collection contains the light meshes"
                                                       + "All vertices of meshes in the collection would have their MSB set",
                                           default="lights")
